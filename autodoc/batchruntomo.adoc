...
tooltip = 
manpage = "Step to start processing with: 0 = setup, 1 = preprocessing, 2 =
xcorr, 3 = prealigned stack, 4 = patch tracking or seeding or RAPTOR, 5 =
beadtrack, 8 = aligned stack, 9 (or 10) = CTF plotting, 10 (or 11) = CTF
correction, 11 (or 9) = erase gold, 12 = filter,
13 = reconstruction, 18 = postprocessing.  Numbers in parentheses apply if the
\fB-erase\fR option is entered.  Each set will start at this step unless the
...
[Field = EraseBeforeCTF]
short = erase
type = B
usage = Run gold erasing before CTF correction
tooltip =
manpage = Run gold erasing before CTF correction.  This is not the
theoretically correct order for these operations but may be more convenient if
manual intervention is planned.
