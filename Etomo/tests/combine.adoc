...
[[if = test-gui]]
  ...
  assert.rb.use-matching-models-only=off
  assert.cb.load-binned-by-2=off
  if.not.var.patching-tracking.assert.cb.specify-corresponding-points-instead-of-using-coordinate-file=off
  if.not.var.patching-tracking.assert.tf.starting-points-to-use-from-a=
  if.var.patching-tracking.assert.tf.corresponding-fiducial-list-a=
  if.var.patching-tracking.assert.assert.tf.corresponding-fiducial-list-b=
