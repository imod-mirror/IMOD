...
if.var.zero-bead.wait.process.autofidseed=failed
[[if = test-gui]]
  #test values
...
if.var.zero-bead.wait.popup.comscript-terminated=OK
if.var.zero-bead.wait.process.tracking-fiducials=failed
if.var.build.return=
#test 2
[[if = test-gui]]
  #Test validation
...
if.not.var.zero-bead.assert.exists.file=%{dataset}%{axis}.fid
#test 4
...
[[if = test-gui]]
  #Test after run
  if.not.var.zero-bead.assert.bn.track-seed-model=on
  if.not.var.zero-bead.assert.bn.track-with-fiducial-model-as-seed=on
[[]]
#fix dataset
