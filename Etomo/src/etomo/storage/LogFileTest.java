...
public class LogFileTest extends TestCase {
  ...
  public void testOpenReader() throws LogFile.LockException, FileNotFoundException {
    ...
    assertFalse(
      ...
      readerId4.isEmpty());
    assertTrue("A successful openReader() call should open the log file reader.",
      test.isOpen(LogFile.LockType.READ, readerId4));
    try {
      test.backup();
  ...
  public void testOpenForWriting() throws LogFile.LockException, FileNotFoundException,
    ...
    assertFalse(
      ...
      writingId1.isEmpty());
    assertTrue("A successful openForWriting call should open log file for writing.",
  ...
  public void testOpenWriter() throws LogFile.LockException, FileNotFoundException,
    ...
    assertFalse(
      "Should be able to open an log file for writing when it is open for reading.",
      writerId1.isEmpty());
    assertTrue("A successful openWriter call should open log file for writing.",
      test.isOpen(LogFile.LockType.WRITE, writerId1));
  ...
  public void testCloseForWriting() throws LogFile.LockException, IOException {
    LogFile test = getInstance();
    LogFile.WritingId writingId = test.openForWriting();
    assertTrue("Should be able to close an opened log file.",
      test.closeForWriting(writingId));
    assertFalse(
  ...
  public void testIsOpen() throws LogFile.LockException, FileNotFoundException,
    ...
    assertFalse("IsOpen should return false with nothing is opened.",
      test.isOpen(LogFile.LockType.WRITE, readerId));
    assertFalse("IsOpen should return false with nothing is opened.",
      test.isOpen(LogFile.LockType.WRITE, writingId));
    readerId = test.openReader();
    assertTrue("IsOpen should return true after openForReading() is called.",
      test.isOpen(LogFile.LockType.READ, readerId));
    assertFalse("The LockType must match the type of open.",
      test.isOpen(LogFile.LockType.WRITE, readerId));
    assertTrue(test.closeRead(readerId));
    ...
    assertTrue("IsOpen should return true after openForWriting() is called.",
      test.isOpen(LogFile.LockType.WRITE, writingId));
    assertFalse("The LockType must match the type of open.",
      test.isOpen(LogFile.LockType.READ, writingId));
    readerId = test.openReader();
    assertTrue("IsOpen should return true after openForReading() is called.",
      test.isOpen(LogFile.LockType.READ, readerId));
    ...
    assertTrue(test.closeRead(readerId));
    assertTrue(test.closeForWriting(writingId));
    assertFalse("IsOpen should return false when log file is closed.",
      test.isOpen(LogFile.LockType.READ, readerId));
    assertFalse("IsOpen should return false when log file is closed.",
      test.isOpen(LogFile.LockType.WRITE, writingId));
    LogFile.WriterId writerId = test.openWriter();
  ...
  public void testInputStream() throws LogFile.LockException, IOException {
    ...
    assertTrue("Should be able to load with input stream open", props.getProperty(key)
      .equals(value));
    assertTrue("Should be able to close an open input stream.",
      test.closeInputStream(inputStreamId));
    assertFalse("Should be able to close a closed input stream.",
      test.closeInputStream(inputStreamId));
    assertTrue(test.noLocks());
  }
  ...
  public void testOutputStream() throws LogFile.LockException, FileNotFoundException,
    ...
    assertTrue("Should be able to close an open output stream.",
      test.closeOutputStream(outputStreamId));
    assertFalse("Should not be able to close a closed output stream.",
      test.closeOutputStream(outputStreamId));
    assertTrue(test.noLocks());
  }
  ...
  public void testIds() throws LogFile.LockException, FileNotFoundException, IOException {
    ...
    assertEquals("Ids in different instances do not affect each other", id0a.get(),
      id0b.get());
  ...
  private void assertWriteLock(LogFile test, LogFile.Id testWriteId)
    ...
    assertTrue("The WRITE lock should be set",
      test.isOpen(LogFile.LockType.WRITE, testWriteId));
    try {
      test.backup();
  ...
  private LogFile getInstance() throws LogFile.LockException {
    LogFile logFile =
      LogFile.getInstance(testDir.getAbsolutePath(), AxisID.ONLY, ProcessName.BLEND);
    return logFile;
  }
