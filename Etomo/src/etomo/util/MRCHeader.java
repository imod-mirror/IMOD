...
public class MRCHeader {
  ...
  public static MRCHeader getInstance(BaseManager manager, AxisID axisID,
      String fileExt) {
    return MRCHeader.getInstance(manager.getPropertyUserDir(),
        DatasetFiles.getDatasetFile(manager, axisID, fileExt).getAbsolutePath(), axisID);
  }

  public static MRCHeader getInstanceFromFileName(BaseManager manager, AxisID axisID,
      String fileName) {
    return MRCHeader.getInstance(manager.getPropertyUserDir(),
        DatasetFiles.getDatasetFileFromFileName(manager, axisID, fileName)
  ...
  public static MRCHeader getInstance(BaseManager manager, AxisID axisID,
      FileType fileType) {
    File keyFile = Utilities
        .getFile(manager.getPropertyUserDir(), fileType.getFileName(manager, axisID));
  ...
  public synchronized boolean read(BaseManager manager)
    ...
    if (!file.exists()) {
      System.err.println(
          "WARNING: attempting to read the header of " + file.getAbsolutePath() +
              ", which doesn't exist.");
      return false;
    }
    ...
    for (int i = 0; i < stdOutput.length; i++) {
      ...
      if (stdOutput[i].trim().startsWith(SIZE_HEADER)) {
        ...
        try {
          ...
          throw new NumberFormatException(
              "nRows not set, token is " + tokens[N_ROWS_INDEX]);
        }
        try {
          ...
          Utilities.timestamp("read", "header", filename, Utilities.FAILED_STATUS);
          throw new NumberFormatException(
              "nSections not set, token is " + tokens[N_SECTIONS_INDEX]);
        }
      }
      ...
      // comment section
      if (pixelEquals(1.0) || pixelEquals(2.0) || pixelEquals(4.0)) {
  ...
  private boolean pixelEquals(final double size) {
    return xPixelSize.equals(size) && yPixelSize.equals(size) && yPixelSize.equals(size);
  }
    ...
    if (!pixelSpacing.isValid() || dPixelSpacing == -1 || dPixelSpacing == 0) {
      if (popupErrorMessage) {
        UIHarness.INSTANCE.openMessageDialog(manager,
            "Invalid pixel spacing:  " + sPixelSpacing + ".  Fix the mrc header in " +
                filename + " with alterheader.", "Header Error", axisID);
      }
      return false;
    }
    return true;
