...
static int tltfopen(ImodView *vw, char *ff)
{
  ...
  xtilt->white = 255;
  xtilt->movie = False;
  xtilt->cx = (int)vw->xmouse;
  xtilt->cy = (int)vw->ymouse;
  xtilt->cz = (int)(vw->zmouse + 0.5);
  ...
  if (xtilt->isModelImage){
    ...
    if (App->depth == 8){
      ...
      for(k = 0; k < ksize; k++)
        for(i = 0; i < xysize; i++){
          pix = (unsigned char)(idata[k][i] * scale + rbase);
          idata[k][i] = pix;
        }
    }
  }
...
static void ginit_cb(Widget w, XtPointer client, XtPointer call)
{
  ...
  xtilt->cx = (int)xtilt->vi->xmouse;
  xtilt->cy = (int)xtilt->vi->ymouse;
  xtilt->cz = (int)(xtilt->vi->zmouse + 0.5);
  ...
  for (pt = 0; pt < cont->psize; pt++){
    b3dVertex2i(b3dGetDrawCoord
                (xtiltGetx(xtilt, 
                           ...
                           (int)cont->pts[pt].z),
                 xtilt->zoom, xtilt->xo),
                (int)b3dGetDrawCoord(cont->pts[pt].y,xtilt->zoom,xtilt->yo));
    /*
  ...
  if (xtilt->stereo == 1){
    ...
    for (pt = 0; pt < cont->psize; pt++){
      b3dVertex2i((xtilt->width/2) + 
                  ...
                  (int)b3dGetDrawCoord(cont->pts[pt].y,
                     ...
                     double tilt, int x, int z)
{
  ...
  /* rotate */
  xpos = (int)((tx * cos(tdeg)) + (tz * sin(tdeg)));
