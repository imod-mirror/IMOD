...
!
! Modules: NDA, NDMODEL, GRAPHDEN, ANGDIST, NDRANDOM, NDSUBS,
! NDMTKRAND, NDMTKSUBS, CONVEXBOUND
!
...
subroutine realGraphicsMain()
  implicit none
  integer LIMBINS, LIMGRAPHS, LIMPNTS, LIMRAND, LIMREGION, LIMTYPE, LIMVERT, ITYPEALL
  integer NOPTNEEDMODEL
  parameter (LIMGRAPHS = 50, LIMBINS = 1001, LIMPNTS = 50000, &
      LIMVERT = 5000, LIMREGION = 200, ITYPEALL = 999)
  parameter (LIMTYPE = 50, LIMRAND = 1000)
  ...
  integer*4 itypeShift(LIMTYPE), itypeBound(LIMTYPE)
  !
  integer*4 numInClass(LIMTYPE,LIMREGION), numInTmp(LIMREGION)
  ...
  real*4 zzRegion(LIMREGION), wholeAreaRegion(LIMREGION)
  ...
  real*4 probNear(LIMBINS), probNearSave(LIMBINS), probNearIn(LIMBINS)
  ...
  common /nmsinput/ in5
  !
  real*4 wholeArea, areaSum, avgden, avgint, base, baseVal, boundDistMin, c1, c2, c3
  real*4 countSum, deltaNear, deltaRad, deltaNearSave, dens, density, dyTmp, err, fracOmit
  real*4 padBound, padBoundIn, pctAbove, power, randInteg, range, radMax, centroid
  real*4 radMin, sdDen, sdIntegral, semDen, sum, binSum, binSumSq, to, trueavg, x, xmax
  ...
  integer*4 ifSample, ifScale, ifShuffle, ifSpecial, ifUseSave, ifxy, ii
  ...
  integer*4 jgrSd, lastAngDiff, numInTot, numPoints, numVert
  integer*4 lineEnd, lineStart, maxGraph, maxTmp, nbinAvg, numBins
  integer*4 numNearBinsSave, numChange, numDisp, numControlDo, numExtraGraph, numGraphs
  integer*4 numRegions, numNearBins, numTotControl, numTypeBound, numTypes, numTypeShift
