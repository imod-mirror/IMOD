...
program blendmont
  ...
  !
  ! fallbacks from ../../manpages/autodoc2man -2 2 blendmont
  integer numOptions
...
54 if (modeParallel < 0 .and. (ifOldEdge == 0 .or. edgesIncomplete)) &
  ...
  if (numXcorrPeaks > 1 .and. .not. xcLegacy) then
    ...
    if (undistortOnly) then
      ...
      do ipc = 1, npcList
        if (izPcList(ipc) == izSect) then
          ...
          !
          if (outputpl) write(3, '(2i6,i4)') newPcXlowLeft, newPcYlowLeft, izSect
        endif
      enddo
...
CONTAINS
  ...
  subroutine getBestPieceShifts()
    ...
    write(*,'(i5,a,2f7.1,a,a,2f7.2)') numBestEdge, &
