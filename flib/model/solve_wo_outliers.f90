...
subroutine do3multr(xMat, matCols, numData, numColIn, numFit, icolFixIn, aMat3d, delXYZ, &
  ...
  common /funccom/ nullAxis, magSign, ifTrace, numTrials, errMin, sumSq, aa
  real*8 sum
  ...
  if (icolFixed == 0) then
    ...
    ! subtract that column's independent var from its dependent var
    ! and pack the dependent vars into the smaller number of columns
    !
    numColDo = numColIn - 1
    do i = 1, numFit
